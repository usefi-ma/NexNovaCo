...
On the next Development startup, the additive Home Statistics migration applies to the same database and initializes the approved four items only if the collection is empty. A partial collection is not automatically repaired. Existing Admin accounts and saved Hero/Welcome/Services/Projects/Team/Statistics edits are retained. Stop/restart an existing Visual Studio debugging session to load the new build; no database reset is needed. The public site still uses its approved design and remains anonymous. Outside Development, the existing controlled database-initialization opt-in is unchanged.
...
`IHomeContentService.GetAsync` supplies typed Home data. The scoped implementation reads Hero, Welcome, Services intro, Projects intro, Team intro, and Statistics from their focused SQLite content services on each call, outside the static-content snapshot. Project/member cards still come from `IProjectCatalog` and `IMemberCatalog` with approved featured IDs; other sections retain their approved static sources. Restart after changing JSON. Member records, card order, images, social links, and the separate Team page are not editable here. Existing fixed-height Team styling remains unchanged: keep edited copy concise and preview Home after saving. Statistics retains its existing CountUp enhancement and markup; labels are required (32 characters maximum), values are whole numbers from 0 to 9,999, and the four approved positions remain fixed. This is not a generic CMS.

## Next phase — approval required

Recommended next Dashboard slice: Partners introductory content, subject to separate approval and inspection of its shared Home/About provider. It is not implemented. Partner cards and Testimonials remain deferred; member records and featured ordering remain catalog-driven. Service/project/member CRUD, other CMS sections, media upload, drafts/versioning, email/newsletter delivery, and merge/deployment remain out of scope. Keep the static reference.
