...
`IHomeContentService.GetAsync` supplies typed Home data. The scoped implementation reads Hero, Welcome, Services intro, Projects intro, Team intro, Statistics, and Partners intro from their focused SQLite content services on each call, outside the static-content snapshot. Project/member cards still come from `IProjectCatalog` and `IMemberCatalog` with approved featured IDs. Home and About retain the same `PartnerCatalog.All` collection; only Home's Partners heading/description come from SQLite. Other sections retain their approved static sources. Restart after changing JSON. Member records, card order, images, social links, and the separate Team page are not editable here. Existing fixed-height Team styling remains unchanged: keep edited copy concise and preview Home after saving. Statistics retains its existing CountUp enhancement and markup; labels are required (32 characters maximum), values are whole numbers from 0 to 9,999, and the four approved positions remain fixed. This is not a generic CMS.

## Next phase — approval required

Recommended next Dashboard slice: Testimonials, subject to separate approval and inspection of its shared Home/Projects catalog and carousel boundary. It is not implemented. Partner CRUD and About Partners CMS remain deferred; member records and featured ordering remain catalog-driven. Service/project/member CRUD, other CMS sections, media upload, drafts/versioning, email/newsletter delivery, and merge/deployment remain out of scope. Keep the static reference.
