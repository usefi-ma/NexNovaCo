...
On the next Development startup, the additive Home Testimonials presentation migration applies to the same database and initializes the approved title/description only if its singleton is missing. Existing Admin accounts and saved Hero/Welcome/Services/Projects/Team/Statistics/Partners/Testimonials edits are retained. Stop/restart an existing Visual Studio debugging session to load the new build; no database reset is needed. The public site still uses its approved design and remains anonymous. Outside Development, the existing controlled database-initialization opt-in is unchanged.
...
Projects reuses `InnerPageHero`, `ProjectCard` and the shared `TestimonialsSection`/`TestimonialCard`/`ThemeCarousel`. `TestimonialCatalog.All` holds the identical approved Home/Projects quotes. Projects still takes static brand copy from `TestimonialCatalog.Brand`; Home's separate presentation singleton initializes from that same approved copy. No quote catalog was duplicated. The listing uses the original three/two/one-column grid. Pagination preserves the sample appearance as hidden-from-assistive-technology spans, with no links, buttons, focus stops or page-changing state. No filtering/search was added.
