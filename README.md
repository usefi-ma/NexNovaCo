...
The public-site migration and Phase 9 hardening are merged into `main`. All eight approved public page types remain anonymous on .NET 10 / Interactive Server / **MudBlazor 9.10.0**. The long-lived `feature/dashboard-cms` branch adds ASP.NET Core Identity, EF Core/SQLite, static HTTP login/logout, an Admin-only `/dashboard`, eight Home section editors, and shared Testimonials CRUD/reordering. Home and Projects read the same ordered SQLite testimonial collection. Other catalog cards and the public design remain unchanged. See [Shared Testimonials verification](docs/dashboard-shared-testimonials.md), [Home Testimonials verification](docs/dashboard-phase-4g-home-testimonials.md), [Home Partners verification](docs/dashboard-phase-4f-home-partners.md), [Home Statistics verification](docs/dashboard-phase-4e-home-statistics.md), [Home Team intro verification](docs/dashboard-phase-4d-home-team-intro.md), [Home Projects intro verification](docs/dashboard-home-projects-intro.md), [Home Services intro verification](docs/dashboard-phase-4b-home-services-intro.md), [authentication setup](docs/dashboard-phase-1-authentication.md), and the earlier [production-hardening report](docs/phase-9-production-hardening.md).

## Run locally

The Dashboard has a responsive MudBlazor application shell and avatar menu. Home is a collapsible sidebar group with independent Admin-only editors at `/dashboard/content/home/hero`, `/dashboard/content/home/welcome`, `/dashboard/content/home/services`, `/dashboard/content/home/projects`, `/dashboard/content/home/team`, `/dashboard/content/home/statistics`, `/dashboard/content/home/partners`, and `/dashboard/content/home/testimonials`; there are no tabs. Both `/dashboard/content/home` and the old `/dashboard/home/hero` redirect to canonical Hero. All eight forms share unsaved-edit protection: Stay/Leave for Dashboard navigation and browser-native unload protection for public-site actions, refresh, and close. Services, Projects, and Partners edit only their existing title and description. Partners edits affect Home only; About's intro and the shared six-partner catalog stay unchanged. Testimonials edits only Home's existing brand-panel title/description; Projects and shared quotes/authors remain unchanged. That decorative panel is hidden at 993px and below by the approved public design. Team edits its existing title, introduction, highlight, description, and CTA text/route. Statistics edits the four existing labels and integer values together, without adding, deleting, or reordering items. No member/card controls or featured-selection controls are exposed. See [Team intro verification](docs/dashboard-phase-4d-home-team-intro.md), [Phase 4A browser limitations](docs/dashboard-phase-4a-home-navigation.md), and [Dashboard UI redesign](docs/dashboard-ui-redesign.md).
...
Shared Content → Testimonials is separate from Home → Testimonials Section. Manage the shared collection at `/dashboard/content/testimonials`, add at `/dashboard/content/testimonials/new`, and edit at `/dashboard/content/testimonials/{id:int}`. The form edits the existing combined author/role/company attribution and plain-text quote, with blank lines separating paragraphs. Move up/down persists immediately; deletion requires confirmation. Add/Edit reuses the same unsaved-change guard.

On the next Development startup, additive migration `20260922212453_AddTestimonialInitializationState` records one-time collection initialization in the same database. Defaults seed only for an uninitialized collection; the marker and default rows commit together. Existing legacy collections, including those emptied by Admin deletion, are recognized during upgrade using existing rows or SQLite's retained AUTOINCREMENT history. Deleting one or all testimonials stays deleted across restarts; an empty collection hides both public sections until an Admin adds content. Existing Admin accounts and all Home section edits are retained. Stop/restart an existing Visual Studio debugging session to load the new build; no database reset is needed. Outside Development, the existing controlled database-initialization opt-in is unchanged. See [one-time initialization verification](docs/testimonial-initialization-fix.md).
...
`IHomeContentService.GetAsync` supplies typed Home data. The scoped implementation reads Hero, Welcome, Services intro, Projects intro, Team intro, Statistics, Partners intro, and the Testimonials brand panel from their focused SQLite content services on each call, outside the static-content snapshot. Project/member cards still come from `IProjectCatalog` and `IMemberCatalog` with approved featured IDs. Home and About retain the same `PartnerCatalog.All` collection; only Home's Partners heading/description come from SQLite. Home and Projects both read the shared ordered `Testimonials` table through `ITestimonialContentService` on every call. Home's `TestimonialBrand` remains a separate settings singleton. Other content retains its approved static sources. Restart after changing JSON. Member records, card order, images, social links, and the separate Team page are not editable here. Existing fixed-height Team and Testimonials styling remains unchanged: keep edited copy concise and preview Home after saving. Statistics retains its existing CountUp enhancement and markup; labels are required (32 characters maximum), values are whole numbers from 0 to 9,999, and the four approved positions remain fixed. This is not a generic CMS.

## Next phase — approval required

Recommended next Dashboard milestone: separately scope shared Partner CRUD across Home/About, using this focused collection pattern and explicit logo-path validation without uploads. Partner CRUD and About Partners CMS are not implemented; member records and featured ordering remain catalog-driven. Service/project/member CRUD, other CMS sections, media upload, drafts/versioning, email/newsletter delivery, and merge/deployment remain out of scope. Keep the static reference.
...
`IAboutContentService.GetAsync` supplies typed hero, story, vision, timeline and mission content from an in-memory snapshot of approved `about.html`. `PartnerCatalog` supplies the same six partner identities/copy/images to Home and About. About still uses its approved static heading/description; Home uses a separate two-field CMS intro initialized from that approved copy. Both routes use `PartnersSection`, `PartnerCard` and `ThemeCarousel`; existing component APIs are unchanged. No partner destination was invented.
...
Projects reuses `InnerPageHero`, `ProjectCard` and the shared `TestimonialsSection`/`TestimonialCard`/`ThemeCarousel`. Its now-scoped provider reads the same SQLite testimonials as Home, outside its immutable project/hero snapshot. `TestimonialCatalog.All` is used only for controlled one-time initialization and logged read-failure fallback; it is no longer normal production data. Public fallback performs no writes, and a valid empty collection remains empty. Projects still takes static brand copy from `TestimonialPresentationDefaults.Brand`; Home's separate presentation singleton initializes from that same approved copy. No quote catalog was duplicated. Public testimonial markup/CSS/JS are unchanged when items exist; an empty collection does not mount an empty carousel. The listing uses the original three/two/one-column grid. Pagination preserves the sample appearance as hidden-from-assistive-technology spans, with no links, buttons, focus stops or page-changing state. No filtering/search was added.
