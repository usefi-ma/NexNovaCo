...
The public-site migration and Phase 9 hardening are merged into `main`. All eight approved public page types remain anonymous on .NET 10 / Interactive Server / **MudBlazor 9.10.0**. The long-lived `feature/dashboard-cms` branch adds ASP.NET Core Identity, EF Core/SQLite, static HTTP login/logout, an Admin-only `/dashboard`, Home Hero/Welcome text and CTA editing, and Home Services/Projects/Team intro editing. Service/project/member cards and the public design remain unchanged. See [Home Team intro verification](docs/dashboard-phase-4d-home-team-intro.md), [Home Projects intro verification](docs/dashboard-home-projects-intro.md), [Home Services intro verification](docs/dashboard-phase-4b-home-services-intro.md), [authentication setup](docs/dashboard-phase-1-authentication.md), and the earlier [production-hardening report](docs/phase-9-production-hardening.md).

## Run locally

The Dashboard has a responsive MudBlazor application shell and avatar menu. Home is a collapsible sidebar group with independent Admin-only editors at `/dashboard/content/home/hero`, `/dashboard/content/home/welcome`, `/dashboard/content/home/services`, `/dashboard/content/home/projects`, and `/dashboard/content/home/team`; there are no tabs. Both `/dashboard/content/home` and the old `/dashboard/home/hero` redirect to canonical Hero. All five forms share unsaved-edit protection: Stay/Leave for Dashboard navigation and browser-native unload protection for public-site actions, refresh, and close. Services and Projects edit only their existing title and description. Team edits its existing title, introduction, highlight, description, and CTA text/route. No member/card controls or featured-selection controls are exposed. See [Team intro verification](docs/dashboard-phase-4d-home-team-intro.md), [Phase 4A browser limitations](docs/dashboard-phase-4a-home-navigation.md), and [Dashboard UI redesign](docs/dashboard-ui-redesign.md).
...
On the next Development startup, the additive Home Team intro migration applies to the same database and initializes approved copy only if its singleton is missing. Existing Admin accounts and saved Hero/Welcome/Services/Projects/Team edits are retained. Stop/restart an existing Visual Studio debugging session to load the new build; no database reset is needed. The public site still uses its approved design and remains anonymous. Outside Development, the existing controlled database-initialization opt-in is unchanged.
...
`IHomeContentService.GetAsync` supplies typed Home data. The scoped implementation reads Hero, Welcome, Services intro, Projects intro, and Team intro from their focused SQLite content services on each call, outside the static-content snapshot. Project/member cards still come from `IProjectCatalog` and `IMemberCatalog` with approved featured IDs; other sections retain their approved static sources. Restart after changing JSON. Member records, card order, images, social links, and the separate Team page are not editable here. Existing fixed-height Team styling remains unchanged: keep edited copy concise and preview Home after saving. This is not a generic CMS.

## Next phase — approval required

Recommended next Dashboard slice: Home Statistics labels/counts, subject to separate approval and inspection of its existing structure and counter behavior. It is not implemented; member records and featured ordering remain catalog-driven. Service/project/member CRUD, other CMS sections, media upload, drafts/versioning, email/newsletter delivery, and merge/deployment remain out of scope. Keep the static reference.
