...
On the next Development startup, additive migration `20260922203426_AddSharedTestimonials` creates the shared collection in the same database. Controlled startup seeds the two exact approved testimonials only when the collection is empty; it never overwrites a nonempty collection. Existing Admin accounts and all Home section edits are retained. Deleting every testimonial hides both public sections until a new item is added; the next controlled startup reseeds an empty collection (also explained when deleting the last item). Stop/restart an existing Visual Studio debugging session to load the new build; no database reset is needed. Outside Development, the existing controlled database-initialization opt-in is unchanged.
