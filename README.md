...
The public-site migration and Phase 9 hardening are merged into `main`. All eight approved public page types remain anonymous on .NET 10 / Interactive Server / **MudBlazor 9.10.0**. The long-lived `feature/dashboard-cms` branch adds ASP.NET Core Identity, EF Core/SQLite, static HTTP login/logout, an Admin-only `/dashboard`, Home Hero/Welcome text and CTA editing, and Home Services/Projects intro editing. Service/project cards and the public design remain unchanged. See [Home Projects intro verification](docs/dashboard-home-projects-intro.md), [Home Services intro verification](docs/dashboard-phase-4b-home-services-intro.md), [authentication setup](docs/dashboard-phase-1-authentication.md), and the earlier [production-hardening report](docs/phase-9-production-hardening.md).

## Run locally

The Dashboard has a responsive MudBlazor application shell and avatar menu. Home is a collapsible sidebar group with independent Admin-only editors at `/dashboard/content/home/hero`, `/dashboard/content/home/welcome`, `/dashboard/content/home/services`, and `/dashboard/content/home/projects`; there are no tabs. Both `/dashboard/content/home` and the old `/dashboard/home/hero` redirect to canonical Hero. All four forms share unsaved-edit protection: Stay/Leave for Dashboard navigation and browser-native unload protection for public-site actions, refresh, and close. Services and Projects edit only their existing title and description, with no eyebrow, CTA, card controls, or featured-selection controls. See [Projects intro verification](docs/dashboard-home-projects-intro.md), [Phase 4A browser limitations](docs/dashboard-phase-4a-home-navigation.md), and [Dashboard UI redesign](docs/dashboard-ui-redesign.md).
...
Recommended next Dashboard slice: Home Team introductory content, subject to separate approval and inspection of its existing fields/CTA. It is not implemented; member records and featured ordering should remain catalog-driven. Service/project CRUD, other CMS sections, media upload, drafts/versioning, email/newsletter delivery, and merge/deployment remain out of scope. Keep the static reference.
