...
The public-site migration and Phase 9 hardening are merged into `main`. All eight approved public page types remain anonymous on .NET 10 / Interactive Server / **MudBlazor 9.10.0**. The long-lived `feature/dashboard-cms` branch adds ASP.NET Core Identity, EF Core/SQLite, static HTTP login/logout, an Admin-only `/dashboard`, Home Hero/Welcome text and CTA editing, and Home Services intro editing. Service cards and the public design remain unchanged. See [Home Services intro verification](docs/dashboard-phase-4b-home-services-intro.md), [Home CMS navigation](docs/dashboard-phase-4a-home-navigation.md), [authentication setup](docs/dashboard-phase-1-authentication.md), and the earlier [production-hardening report](docs/phase-9-production-hardening.md).

## Run locally

The Dashboard has a responsive MudBlazor application shell and avatar menu. Home is a collapsible sidebar group with independent Admin-only editors at `/dashboard/content/home/hero`, `/dashboard/content/home/welcome`, and `/dashboard/content/home/services`; there are no tabs. Both `/dashboard/content/home` and the old `/dashboard/home/hero` redirect to canonical Hero. All three forms share unsaved-edit protection: Stay/Leave for Dashboard navigation and browser-native unload protection for public-site actions, refresh, and close. Services edits only the existing title and description, with no eyebrow, CTA, or service-card controls. See [Phase 4B verification](docs/dashboard-phase-4b-home-services-intro.md), [Phase 4A browser limitations](docs/dashboard-phase-4a-home-navigation.md), and [Dashboard UI redesign](docs/dashboard-ui-redesign.md).
...
On the next Development startup, the additive Home Services intro migration applies to the same database and initializes approved copy only if its singleton is missing. Existing Admin accounts and saved Hero/Welcome/Services edits are retained. Stop/restart an existing Visual Studio debugging session to load the new build; no database reset is needed. The public site still uses its approved design and remains anonymous. Outside Development, the existing controlled database-initialization opt-in is unchanged.
...
Recommended next Dashboard slice: Home Projects introductory title/description, subject to separate approval and inspection of its existing structure. It is not implemented; project records and featured ordering should remain catalog-driven. Service CRUD, other CMS sections, media upload, drafts/versioning, email/newsletter delivery, and merge/deployment remain out of scope. Keep the static reference.
