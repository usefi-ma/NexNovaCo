...
public sealed class HomePartnersSectionContentService(IDbContextFactory<ApplicationDbContext> factory,
    AuthenticationStateProvider authentication, IOptions<IdentityOptions> identityOptions,
    ILogger<HomePartnersSectionContentService> logger) : IHomePartnersSectionContentService
{
    public async Task<SectionHeading> GetAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var settings = await database.HomePartnersSectionSettings.AsNoTracking()
                .SingleOrDefaultAsync(x => x.Id == HomePartnersSectionSettings.SingletonId, cancellationToken);
            if (settings is not null)
            {
                var content = settings.ToContent();
                var model = HomePartnersSectionEditModel.FromContent(content);
                Validator.ValidateObject(model, new ValidationContext(model), validateAllProperties: true);
                return content;
            }
            logger.LogWarning("Home Partners section record is missing; rendering approved defaults without writing to the database.");
        }
        catch (Exception exception) when (exception is DbException or ValidationException)
        {
            logger.LogError(exception, "Home Partners section could not be read safely; rendering approved defaults without writing to the database.");
        }
        return HomePartnersSectionDefaults.Content;
    }

    public async Task<HomePartnersSectionEditModel> GetForEditAsync(CancellationToken cancellationToken = default)
    {
        await using var database = await factory.CreateDbContextAsync(cancellationToken);
        await RequireAdminAsync(database, cancellationToken);
        var settings = await database.HomePartnersSectionSettings.AsNoTracking()
            .SingleOrDefaultAsync(x => x.Id == HomePartnersSectionSettings.SingletonId, cancellationToken)
            ?? throw new KeyNotFoundException("Home Partners section has not been initialized.");
        // The editor must report read failures, never present fallback as if it were saved content.
        return HomePartnersSectionEditModel.FromContent(settings.ToContent());
    }

    public async Task UpdateAsync(HomePartnersSectionEditModel model, CancellationToken cancellationToken = default)
    {
        await using var database = await factory.CreateDbContextAsync(cancellationToken);
        await RequireAdminAsync(database, cancellationToken);
        Validator.ValidateObject(model, new ValidationContext(model), validateAllProperties: true);
        var settings = await database.HomePartnersSectionSettings.SingleOrDefaultAsync(x => x.Id == HomePartnersSectionSettings.SingletonId, cancellationToken)
            ?? throw new KeyNotFoundException("Home Partners section has not been initialized.");
