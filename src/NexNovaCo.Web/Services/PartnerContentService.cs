    ...
    ILogger<PartnerContentService> logger, IMediaStorageService? media = null) : IPartnerContentService
{
    public async Task<IReadOnlyList<Partner>> GetAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogError(exception, "Partners could not be read safely; rendering approved fallback without database writes.");
            return PartnerCatalog.All;
        }
    }

    public async Task<IReadOnlyList<PartnerListItem>> ListAsync(CancellationToken cancellationToken = default)
    {
        await using var database = await factory.CreateDbContextAsync(cancellationToken);
        await RequireAdminAsync(database, cancellationToken);
        return await Ordered(database).AsNoTracking()
            .Select(row => new PartnerListItem(row.Id, row.DisplayOrder, row.Name, row.ImagePath, row.HasLogoBackground, row.Href)).ToListAsync(cancellationToken);
    }

    public async Task<PartnerEditModel> GetForEditAsync(int id, CancellationToken cancellationToken = default)
    {
        await using var database = await factory.CreateDbContextAsync(cancellationToken);
        await RequireAdminAsync(database, cancellationToken);
        var row = await database.Partners.AsNoTracking().SingleOrDefaultAsync(row => row.Id == id, cancellationToken)
            ?? throw new KeyNotFoundException("Partner no longer exists.");
        return row.ToEditModel();
    }

    public async Task<int> CreateAsync(PartnerEditModel model, CancellationToken cancellationToken = default)
    {
        await using var database = await factory.CreateDbContextAsync(cancellationToken);
        ...
        database.Partners.Add(row);
        await database.SaveChangesAsync(cancellationToken);
        await transaction.CommitAsync(cancellationToken);
        return row.Id;
    }

    public async Task UpdateAsync(int id, PartnerEditModel model, CancellationToken cancellationToken = default)
    {
        await using var database = await factory.CreateDbContextAsync(cancellationToken);
        await RequireAdminAsync(database, cancellationToken);
        Validate(model);
    ...
    public async Task DeleteAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        await RequireAdminAsync(database, cancellationToken);
        var rows = await Ordered(database).ToListAsync(cancellationToken);
        var row = rows.SingleOrDefault(row => row.Id == id) ?? throw new KeyNotFoundException("Partner no longer exists.");
        database.Partners.Remove(row);
        rows.Remove(row);
        Normalize(rows);
        await database.SaveChangesAsync(cancellationToken);
        await transaction.CommitAsync(cancellationToken);
    ...
    private static IOrderedQueryable<PartnerEntity> Ordered(ApplicationDbContext database) =>
        database.Partners.OrderBy(row => row.DisplayOrder).ThenBy(row => row.Id);
    private static void Validate(PartnerEditModel model) =>
        Validator.ValidateObject(model, new ValidationContext(model), validateAllProperties: true);
    private static void Normalize(IReadOnlyList<PartnerEntity> rows)
    {
