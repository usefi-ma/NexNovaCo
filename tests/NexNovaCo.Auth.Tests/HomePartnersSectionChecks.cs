...
internal static class HomePartnersSectionChecks
{
    public static async Task RunAsync()
    {
        await using var app = new AuthFactory(NewPassword());
        using var anonymous = app.NewClient();
        var denied = await anonymous.GetAsync("/dashboard/content/home/partners");
        Check(denied.StatusCode == HttpStatusCode.Redirect && denied.Headers.Location!.ToString().Contains("/admin/login"), "Anonymous PartnersSection editor must challenge.");
        using var adminClient = app.NewClient();
        await Login(adminClient, AuthFactory.Email, app.Password);
        var editor = await adminClient.GetAsync("/dashboard/content/home/partners");
        var editorHtml = await editor.Content.ReadAsStringAsync();
        Check(editor.StatusCode == HttpStatusCode.OK && editorHtml.Contains("Our Partners") && !editorHtml.Contains("Opening line"), "Admin editor must load saved content.");
        Check(editorHtml.Contains("mud-input") && editorHtml.Contains("\"type\":\"server\""), "PartnersSection editor must use interactive Mud inputs.");
        Check((await adminClient.GetAsync("/dashboard/content/home/partners")).StatusCode == HttpStatusCode.OK, "Partners editor must support direct refresh.");
        Check(editorHtml.Contains("href=\"/dashboard/content/home/partners\"") && editorHtml.Contains("Home / Partners"), "Partners editor must have its own sidebar link and page context.");
        Check(typeof(HomePartnersSectionEditModel).GetProperties().Select(x => x.Name).Order().SequenceEqual(new[] { "Description", "Title" }), "Only the two actual intro fields may be editable.");
        ...
        await using (var scope = app.Services.CreateAsyncScope())
        {
            ...
            Check((await users.CreateAsync(new ApplicationUser { UserName = "partners-viewer@example.invalid", Email = "partners-viewer@example.invalid" }, viewerPassword)).Succeeded, "CMS non-Admin test setup failed.");
        }
        using var viewerClient = app.NewClient();
        await Login(viewerClient, "partners-viewer@example.invalid", viewerPassword);
        var viewerEditor = await viewerClient.GetAsync("/dashboard/content/home/partners");
        ...
        var service = new HomePartnersSectionContentService(factory, auth, options, NullLogger<HomePartnersSectionContentService>.Instance);
        var initial = await service.GetAsync();
        Check(Same(initial, HomePartnersSectionDefaults.Content), "Initial CMS copy must exactly match approved defaults, including description.");
        await using (var db = await factory.CreateDbContextAsync())
        {
            Check(await db.HomePartnersSectionSettings.CountAsync() == 1, "Startup must create exactly one PartnersSection.");
            Check((await db.HomePartnersSectionSettings.SingleAsync()).Id == 1, "PartnersSection must use fixed Id 1.");
        }
        ...
        foreach (var attempt in Enumerable.Range(0, 2))
        {
            ...
            Check(publicHtml.Contains(edit.Title) && publicHtml.Contains(edit.Description), "Anonymous Home/refresh must read edited PartnersSection from SQLite.");
        }
        await using (var db = await factory.CreateDbContextAsync())
        {
            await HomePartnersSectionInitializer.InitializeAsync(db);
            await HomePartnersSectionInitializer.InitializeAsync(db);
            Check(await db.HomePartnersSectionSettings.CountAsync() == 1 && (await db.HomePartnersSectionSettings.SingleAsync()).Title == edit.Title, "Repeated initialization must not overwrite edits or duplicate singleton.");
            Check((await db.HomePartnersSectionSettings.SingleAsync()).UpdatedAtUtc > DateTime.UtcNow.AddMinutes(-5), "Save must set a recent UTC timestamp.");
            db.HomePartnersSectionSettings.Add(new HomePartnersSectionSettings { Id = 2 });
        ...
        foreach (var property in typeof(HomePartnersSectionEditModel).GetProperties())
        {
            var invalid = HomePartnersSectionEditModel.FromContent(initial);
            property.SetValue(invalid, "   ");
            await ExpectAsync<ValidationException>(() => service.UpdateAsync(invalid), "Every PartnersSection field must reject whitespace.");
            ...
            await ExpectAsync<ValidationException>(() => service.UpdateAsync(invalid), "Every PartnersSection field must enforce its length boundary.");
        }
        ...
        await using (var db = await factory.CreateDbContextAsync())
            await db.HomePartnersSectionSettings.ExecuteDeleteAsync(); // Only this suite's isolated database.
        ...
        await using (var db = await factory.CreateDbContextAsync())
        {
            Check(await db.HomePartnersSectionSettings.CountAsync() == 0, "Public fallback must not persist a record.");
            await HomePartnersSectionInitializer.InitializeAsync(db);
            Check(await db.HomePartnersSectionSettings.CountAsync() == 1, "Controlled initialization must repair a missing record.");
        }
        var unavailable = new HomePartnersSectionContentService(new UnavailableFactory(), auth, options, NullLogger<HomePartnersSectionContentService>.Instance);
        ...
        await using (var db = await factory.CreateDbContextAsync())
            await db.HomePartnersSectionSettings.ExecuteUpdateAsync(set => set.SetProperty(x => x.Title, ""));
        ...
        var encoded = HomePartnersSectionEditModel.FromContent(initial);
        ...
        await using (var scope = app.Services.CreateAsyncScope())
        {
            ...
            Check(homeBefore.Partners.SequenceEqual(homeAfter.Partners) && homeAfter.Partners.Count == 6, "Home partner cards, order, copy, logos and routes must remain canonical.");
        }
        await using (var db = await factory.CreateDbContextAsync())
            Check((await db.HomeServicesSectionSettings.SingleAsync()).ToContent() == servicesBefore, "Partners intro must not modify saved Services settings.");
        await using (var scope = app.Services.CreateAsyncScope())
        {
            var homeAfter = await scope.ServiceProvider.GetRequiredService<IHomeContentService>().GetAsync();
    ...
    {
        // Upgrade the previous Statistics schema, preserving Identity and all six existing Home slices.
        ...
        Check((await db.Users.SingleAsync()).PasswordHash == originalHash && await db.UserRoles.CountAsync() == 1 && await db.Roles.CountAsync() == 1, "PartnersSection upgrade must preserve existing Identity account/hash/role mapping.");
        Check((await db.HomeHeroSettings.SingleAsync()).OpeningLine == "Existing Hero edit", "PartnersSection migration must preserve Hero edits.");
        Check((await db.HomeWelcomeSettings.SingleAsync()).Title == "Existing Welcome edit", "Partners migration must preserve Welcome edits.");
        Check((await db.HomeServicesSectionSettings.SingleAsync()).Title == "Existing Services edit", "Partners migration must preserve Services intro edits.");
        Check((await db.HomeProjectsSectionSettings.SingleAsync()).Title == "Existing Projects edit" &&
            (await db.HomeTeamSectionSettings.SingleAsync()).Title == "Existing Team edit" &&
            (await db.HomeStatistics.SingleAsync(x => x.Id == 1)).Value == 875, "Partners migration preserves the later three Home slices.");
        Check(await db.HomePartnersSectionSettings.CountAsync() == 1 && !db.Database.HasPendingModelChanges(), "Additive migration must create PartnersSection table with no model drift.");
    }
