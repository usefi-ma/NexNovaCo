...
internal static class HomeWelcomeChecks
{
    public static async Task RunAsync()
    {
        ...
        await using (var scope = app.Services.CreateAsyncScope())
        {
            ...
            Check((await users.CreateAsync(new ApplicationUser { UserName = "welcome-viewer@example.invalid", Email = "welcome-viewer@example.invalid" }, viewerPassword)).Succeeded, "CMS non-Admin test setup failed.");
        }
        ...
        var service = new HomeWelcomeContentService(factory, auth, options, NullLogger<HomeWelcomeContentService>.Instance);
        var initial = await service.GetAsync();
        Check(Same(initial, HomeWelcomeDefaults.Content), "Initial CMS copy must exactly match approved defaults, including relative CTA.");
        await using (var db = await factory.CreateDbContextAsync())
        {
            Check(await db.HomeWelcomeSettings.CountAsync() == 1, "Startup must create exactly one Welcome.");
            Check((await db.HomeWelcomeSettings.SingleAsync()).Id == 1, "Welcome must use fixed Id 1.");
        }
        ...
        await using (var db = await factory.CreateDbContextAsync())
            Check((await db.HomeHeroSettings.SingleAsync()).ToContent() == heroBefore, "Welcome save must not alter Hero.");
        foreach (var attempt in Enumerable.Range(0, 2))
        {
            var publicHtml = await anonymous.GetStringAsync("/");
            Check(publicHtml.Contains(edit.Title) && publicHtml.Contains("href=\"/projects\""), "Anonymous Home/refresh must read edited Welcome from SQLite.");
        }
        await using (var db = await factory.CreateDbContextAsync())
        {
            await HomeWelcomeInitializer.InitializeAsync(db);
            await HomeWelcomeInitializer.InitializeAsync(db);
            Check(await db.HomeWelcomeSettings.CountAsync() == 1 && (await db.HomeWelcomeSettings.SingleAsync()).Title == edit.Title, "Repeated initialization must not overwrite edits or duplicate singleton.");
            Check((await db.HomeWelcomeSettings.SingleAsync()).UpdatedAtUtc > DateTime.UtcNow.AddMinutes(-5), "Save must set a recent UTC timestamp.");
            db.HomeWelcomeSettings.Add(new HomeWelcomeSettings { Id = 2 });
        ...
        {
            var invalid = HomeWelcomeEditModel.FromContent(initial);
        ...
        foreach (var property in typeof(HomeWelcomeEditModel).GetProperties())
        {
            var invalid = HomeWelcomeEditModel.FromContent(initial);
            property.SetValue(invalid, "   ");
            await ExpectAsync<ValidationException>(() => service.UpdateAsync(invalid), "Every Welcome field must reject whitespace.");
            ...
            await ExpectAsync<ValidationException>(() => service.UpdateAsync(invalid), "Every Welcome field must enforce its length boundary.");
        }
        ...
        await using (var db = await factory.CreateDbContextAsync())
            await db.HomeWelcomeSettings.ExecuteDeleteAsync(); // Only this suite's isolated database.
        ...
        await using (var db = await factory.CreateDbContextAsync())
        {
            Check(await db.HomeWelcomeSettings.CountAsync() == 0, "Public fallback must not persist a record.");
            await HomeWelcomeInitializer.InitializeAsync(db);
            Check(await db.HomeWelcomeSettings.CountAsync() == 1, "Controlled initialization must repair a missing record.");
        }
        var unavailable = new HomeWelcomeContentService(new UnavailableFactory(), auth, options, NullLogger<HomeWelcomeContentService>.Instance);
        ...
        await using (var db = await factory.CreateDbContextAsync())
            await db.HomeWelcomeSettings.ExecuteUpdateAsync(set => set.SetProperty(x => x.CtaHref, "javascript:alert(1)"));
        ...
        {
            var valid = HomeWelcomeEditModel.FromContent(initial);
            valid.CtaHref = route;
        ...
        var encoded = HomeWelcomeEditModel.FromContent(initial);
        encoded.Title = "<script>alert(1)</script>";
        await service.UpdateAsync(encoded);
        var encodedHtml = await anonymous.GetStringAsync("/");
        Check(encodedHtml.Contains("&lt;script&gt;") && !encodedHtml.Contains(encoded.Title), "Welcome text must remain HTML-encoded.");
        await service.UpdateAsync(HomeWelcomeEditModel.FromContent(initial));
    ...
    {
        // Upgrade the existing Hero/Identity schema, preserving account credentials and edited Hero content.
        ...
        Check((await db.Users.SingleAsync()).PasswordHash == originalHash && await db.UserRoles.CountAsync() == 1 && await db.Roles.CountAsync() == 1, "Welcome upgrade must preserve existing Identity account/hash/role mapping.");
        Check((await db.HomeHeroSettings.SingleAsync()).OpeningLine == "Existing Hero edit", "Welcome migration must preserve Hero edits.");
        Check(await db.HomeWelcomeSettings.CountAsync() == 1 && !db.Database.HasPendingModelChanges(), "Additive migration must create Welcome table with no model drift.");
    }

    private static bool Same(WelcomeContent left, WelcomeContent right) =>
        left.Title == right.Title && left.Introduction == right.Introduction && left.Paragraphs.SequenceEqual(right.Paragraphs) && left.CtaLabel == right.CtaLabel && left.CtaHref == right.CtaHref;
