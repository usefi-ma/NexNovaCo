...
internal static class AuthChecks
{
    ...
    public static async Task Main()
    {
        ...
        Check(!((await challenge.Content.ReadAsStringAsync()).Contains("secure Dashboard foundation")), "Protected content leaked in anonymous response.");
        ...
        foreach (var attempt in Enumerable.Range(0, 2))
        {
            ...
            Check(html.Contains(AuthFactory.Email) && html.Contains("secure Dashboard foundation"), "Dashboard must show authenticated identity/content.");
        ...
        Check(!(await denied.Content.ReadAsStringAsync()).Contains("secure Dashboard foundation"), "Protected content leaked to non-Admin.");
