...
internal static class AuthChecks
{
    ...
    {
        if (args.SequenceEqual(new[] { "--team-page" }))
        {
            try
            {
                await TeamPageCmsChecks.RunAsync();
                Console.WriteLine($"PASS: {_checks} focused Team page checks.");
            }
        ...
        await using (var scope = app.Services.CreateAsyncScope())
        {
            ...
            Check((await db.Database.GetAppliedMigrationsAsync()).Count() == 20, "Expected Identity, approved CMS migrations and additive Home image paths.");
