...
internal static class ProjectsPageCmsChecks
{
    ...
    private static async Task MigrationAsync(IDbContextFactory<ApplicationDbContext> factory)
    {
        ...
        Check(migrations.Length == 19, "One additive Projects migration.");
    ...
    {
        var names = await db.Database.SqlQueryRaw<string>("SELECT name AS Value FROM sqlite_master WHERE type='table' AND name NOT LIKE 'Projects%Settings' AND name NOT LIKE '__EF%' AND name <> 'sqlite_sequence' ORDER BY name").ToArrayAsync();
